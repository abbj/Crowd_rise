...
use utilisateur\utilisateurBundle\Entity\User;
...
class MessageController extends Controller {
    ...
    private function createCreateForm(Message $entity) {
        ...
        $entity->setIdsender($id);
        
        $entity->setNomproprietaire($prenom);
        $entity->setSendAt(new \DateTime("now"));
    ...
    public function mesMessageAction() {
        ...
        #-----------------------------------------
       # $reposotory = $em->getRepository('utilisateurBundle:Message');
        #$query = $reposotory->createQueryBuilder('u');
        #$query->select('u')->where('u.id=:id')->orderBy('u.id')->setParameter('id',$id);
        
        #$g=$query->getQuery()->getResult();
        #-----------------------------------------
        
        $entities = $em->getRepository('utilisateurBundle:Message')->findByidreceiver($id);
       
        return $this->render('utilisateurBundle:Message:mesMessage.html.twig', array(
                    'entities' => $entities
        ));
    }

    public function RepondreAction($toUserId) {
        $entity = new Message();


        $form = $this->createCreateFormRepondre($entity, $toUserId);
        return $this->render('utilisateurBundle:Message:Repondre.html.twig', array(
                    'entity' => $entity,
                    'form' => $form->createView(),
        ));
    }

    private function createCreateFormRepondre(Message $entity, $toUserId) {
        $user = $this->container->get('security.context')->getToken()->getUser();
        $id = $user->getId();
        $prenom = $user->getPrenom();
        $entity->setIdsender($id);
-
        //on va mentionner l id de l utilisateur auquel on  veut l envoyer le message 
        #zieda
        #
 
 $entity->setIdreceiver($toUserId);
        $entity->setNomproprietaire($prenom);
        $entity->setSendAt(new \DateTime("now"));
           
        $form = $this->createForm(new MessageType(), $entity, array(
            'action' => $this->generateUrl('mesMessage'),
            'method' => 'POST',
        ));
